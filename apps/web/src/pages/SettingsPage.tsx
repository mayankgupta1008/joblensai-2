      ...
      <div className="container mx-auto px-4 md:px-8 py-10 md:py-16 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <Button
            variant="outline"
            size="lg"
            className="w-fit h-12 rounded-full font-bold border-emerald-500/20 hover:bg-emerald-500/5 transition-all text-muted-foreground hover:text-emerald-600"
            onClick={() => navigate(-1)}
          >
        ...
        <div className="hidden md:block mb-12">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="w-full justify-start h-14 p-1.5 gap-2 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 backdrop-blur-md">
              {TABS.map((tab) => (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
              ...
              <Separator orientation="vertical" className="mx-2 h-8 bg-emerald-500/10" />
              <Button
                variant="ghost"
        ...
        <div className="md:hidden mb-8">
          <Accordion
            type="single"
            collapsible
            className="w-full border border-emerald-500/10 rounded-2xl bg-background/40 backdrop-blur-xl overflow-hidden shadow-xl"
          >
            <AccordionItem value="settings-tabs" className="border-none">
              ...
              <AccordionContent className="px-3 pb-3">
                <div className="flex flex-col gap-2">
                  {TABS.map((tab) => (
                    <Button
                      ...
                      className={`justify-start h-14 px-4 rounded-xl transition-all border ${
                        activeTab === tab.id
                          ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-600 shadow-sm"
                  ...
                  <Separator className="my-3 mx-2 bg-emerald-500/10" />
                  <Button
                    variant="ghost"
        ...
        <div className="relative group/content animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="absolute -inset-1 bg-linear-to-r from-emerald-500/20 via-blue-500/10 to-emerald-500/20 rounded-[2.5rem] blur-2xl opacity-20 group-hover/content:opacity-30 transition-opacity" />
        ...
        <div className="mt-16 text-center">
          <Badge
            variant="outline"
            className="px-6 py-2 rounded-full border-emerald-500/10 bg-background/40 backdrop-blur-md text-muted-foreground font-medium text-sm shadow-sm"
          >

  ...
  <NavigationMenu>
    <NavigationMenuList className="gap-1">
      <NavigationMenuItem>
        ...
        <NavigationMenuContent>
          <ul className="grid gap-3 p-6 md:w-100 lg:w-125 lg:grid-cols-[.75fr_1fr] bg-background/95 backdrop-blur-xl border border-brand-border rounded-[1.5rem]">
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        sideOffset={8}
        className="w-88 p-0 rounded-[1.5rem] border-brand-border shadow-2xl overflow-hidden bg-background/95 backdrop-blur-xl"
      >
        {/* Header */}
...
const UserMenu = ({ fullName, email, avatar, initials, onLogout }: UserMenuProps) => (
  <DropdownMenu>
    ...
    <DropdownMenuContent
      align="end"
      sideOffset={8}
      className="w-64 rounded-[1.5rem] border-brand-border shadow-2xl p-2 bg-background/95 backdrop-blur-xl"
    >
...
const MobileAuthMenu = ({
  ...
  <div className="space-y-6">
    {/* User summary */}
    <div className="flex items-center gap-4 p-5 rounded-[2rem] bg-emerald-500/[0.03] border border-brand-border">
      <Avatar size="lg" className="ring-2 ring-brand-border">
        {avatar && <AvatarImage src={avatar} alt={fullName} />}
        <AvatarFallback className="bg-emerald-500/10 text-emerald-600 font-black text-lg">
          {initials}

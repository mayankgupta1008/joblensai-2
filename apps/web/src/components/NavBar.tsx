...
interface MobileLinkArgs {
  to: string;
  icon: React.ComponentType<{ className?: string }>;
  children: React.ReactNode;
}

interface MobileAnchorArgs {
  href: string;
  icon: React.ComponentType<{ className?: string }>;
  children: React.ReactNode;
}
...
const NavBar = () => {
  ...
  const getNotificationConfig = (type: Notification["type"]) => {
    switch (type) {
      ...
      case "SUBSCRIPTION_RENEWAL_FAILED":
        return { icon: FaExclamationCircle, tone: "text-red-500 bg-red-500/10" };
      case "SUBSCRIPTION_STARTED":
      case "SUBSCRIPTION_RENEWED":
        return { icon: FaMagic, tone: "text-emerald-500 bg-emerald-500/10" };
      case "JOB_APPLIED":
      case "JOB_INTERVIEW":
        return { icon: FaBriefcase, tone: "text-blue-500 bg-blue-500/10" };
      case "JOB_OFFER":
      case "JOB_ACCEPTED":
        return { icon: FaCheckCircle, tone: "text-emerald-600 bg-emerald-600/10" };
      default:
        return { icon: FaBell, tone: "text-emerald-500 bg-emerald-500/10" };
    }
  };
  ...
  const listItem = ({ className, title, children, icon: Icon, ...props }: ListItemArgs) => (
    <li>
  ...
  const mobileLink = ({ to, icon: Icon, children }: MobileLinkArgs) => (
    <Link
      to={to}
      onClick={closeMobile}
    ...
    >
      ...
      <FaChevronRight className="w-4 h-4 text-muted-foreground opacity-30 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
    </Link>
  );

  const mobileAnchor = ({ href, icon: Icon, children }: MobileAnchorArgs) => (
    <a
      href={href}
      onClick={closeMobile}
      className="flex items-center gap-4 px-4 py-4 rounded-2xl hover:bg-emerald-500/5 transition-all font-bold text-sm group active:bg-emerald-500/10"
          ...
          <NavigationMenuContent>
            <ul className="grid gap-3 p-6 md:w-100 lg:w-125 lg:grid-cols-[.75fr_1fr] bg-background/95 backdrop-blur-xl border border-brand-border rounded-3xl">
              {listItem({
                ...
                title: "AI Outreach",
                icon: FaMagic,
                children: "Automated personalized handshake generated by LLM.",
              })}
              {listItem({
                href: "#features",
                title: "Skill DNA",
                icon: FaFire,
                children: "Matching based on technical skill fingerprints.",
              })}
              {listItem({
                href: "#features",
                title: "Real-time",
                icon: FaBell,
                children: "Instant notifications when you both swipe right.",
              })}
            </ul>
          </NavigationMenuContent>
        </NavigationMenuItem>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-border bg-linear-to-r from-emerald-500/5 via-transparent to-blue-500/5">
          <div className="flex items-center gap-2">
            <FaBell className="w-4 h-4 text-emerald-500" />
          ...
          {notificationItems.length === 0 ? (
            <div className="py-12 text-center px-6">
              <div className="w-14 h-14 rounded-2xl bg-muted/50 flex items-center justify-center mx-auto mb-4">
                <FaBell className="w-6 h-6 text-muted-foreground opacity-50" />
              </div>
  ...
  const mobileAuthMenu = () => (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 p-5 rounded-4xl bg-emerald-500/3 border border-brand-border">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-black text-base tracking-tight truncate">{user?.fullName ?? "User"}</p>
          <p className="text-xs text-muted-foreground truncate font-medium">{user?.email ?? "—"}</p>
        </div>
      </div>

      <div className="space-y-1">
        {mobileLink({ to: "/dashboard", icon: FaThLarge, children: "Dashboard" })}
        {mobileLink({ to: "/notifications", icon: FaBell, children: "Notifications" })}
        {mobileLink({ to: "/upload", icon: FaFire, children: "Swipe jobs" })}
        {mobileLink({ to: "/dashboard", icon: FaUser, children: "Profile" })}
        {mobileLink({ to: "/subscription", icon: FaCreditCard, children: "Subscription" })}
        {mobileLink({ to: "/dashboard", icon: FaCog, children: "Settings" })}
      </div>
  ...
  const mobilePublicMenu = () => (
    <div className="space-y-6">
      <div className="space-y-1">
        {mobileAnchor({ href: "#features", icon: FaMagic, children: "Features" })}
        {mobileAnchor({ href: "#how-it-works", icon: FaChevronRight, children: "How it works" })}
        {mobileAnchor({ href: "#pricing", icon: FaCreditCard, children: "Pricing" })}
        {mobileLink({ to: "/login", icon: FaBriefcase, children: "Post a Job" })}
      </div>
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            >
              ...
              <ScrollArea className="flex-1">
                <div className="p-6">{isAuthenticated ? mobileAuthMenu() : mobilePublicMenu()}</div>
              </ScrollArea>
            </SheetContent>
          </Sheet>
        </div>
